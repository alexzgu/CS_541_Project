...
**Status 2026-07-15 — de-peaking CLOSED (negative ×2), label-offset discovery, new champion.** (a) Training-time label prior (`LABEL_PRIOR_ALPHA`) does NOT de-peak — a per-class log-prior offset is absorbable into the `lm_head` bias; peakiness probe unchanged (blank 0.94, runs median 1 frame). It still helped as a regularizer: `ctc_deprior` = p5mix +4 mixed epochs w/ α=0.3 → greedy 0.169, test SER 0.316, gold SER 0.170. (b) Decode-time prior (`ctc_prior_alpha`, MMS-FA style) + semi-Markov `ctc_decode=viterbi`: monotone in α but hopeless (SER 0.78 @ α=1 vs 0.32 spike) — peaky CTC puts no token mass on non-spike frames, so span scores carry no evidence. Token EXTENTS must come from acoustics (energy/voicing extent refinement of spike segments), not from the DP. (c) Onset-shift sweep exposed a DATA fault: test timed-F1 peaks at shift −4 (0.53) but gold peaks at −1 (0.74) → the t2-extra frozen-test references carry per-song timing offsets (song 92 is the gold-like exception; song 89 is broken outright). A model-free audit (spectral flux × ref-onset train, gold-calibrated) measured the refs **30–130 ms EARLY**: 81 −50, 83 −30, 85 −30, 89 −130, 90 −90, 91 −70, 92 0 ms — matching the sweep per-song. Control songs refute an import-batch fault (original-batch train reads −20..−30, t2-extra train −20..−70): this is corpus-wide source-label variance (goal-3's "±50 ms"), heaviest on songs 89/90/91/70. Shift stays −1; per-song constant correction → SIGNOFFS S12 (`runs/ref_offset_audit.txt`). `ctc_deprior` promoted under gold arbitration (SIGNOFFS S11).
